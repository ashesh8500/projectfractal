...
use egui::{CentralPanel, SidePanel, TopBottomPanel, ScrollArea, RichText, Color32};
                                        ...
                                        for (i, symbol) in history.symbols.iter().enumerate() {
                                            ...
                                            if start < history.prices.len() {
                                                ...
                                                for (j, price) in prices_slice.iter().enumerate() {
                                                    ...
                                                    let open = base_price;
                                                    let close = *price;
                                                    let high = open.max(close) * 1.01; // Add 1% for high
                                                    let low = open.min(close) * 0.99;  // Subtract 1% for low
                                                    ...
                                                    let body_color = if close > open { Color32::GREEN } else { Color32::RED };
                                                    let body_points: PlotPoints = vec![[x - 0.3, open], [x - 0.3, close], [x + 0.3, close], [x + 0.3, open], [x - 0.3, open]].into();
                                                    candle_lines.push(Line::new(body_points).width(2.0).color(body_color).fill(0.0));
                                                }
                                                ...
                                                let points: PlotPoints = prices_slice.iter().enumerate()
                                                    .map(|(j, price)| [j as f64, *price])
                                                    .collect();
                                                plot_ui.line(Line::new(points).name(symbol).width(1.5).color(Color32::BLUE));
                                            }
                                        }
                                    });
                            }
